...
As long as all the incoming events carry some kind of correlatable information that can be used or converted as a sequence key, you can probably make use of a conseq (and safeguard your beer). On the API level the sequence key can be any type of Object but good choices are identifiers that, after hashing, can group related events into the same hash code, and unrelated events into different hash codes. Some examples of the seqence key are order id, shipment id, ticket reservation id, session id, etc.... The default hashing algorithm is from the Guava library; that should be good enough but for those who have PhDs on hashing, you can provide your own hasher as in `ConcurrentSequentialExecutors.newBuilder().withBucketHasher(myConsistentHasher)` instead of `ConcurrentSequentialExecutors.newBuilder().withMaxConcurrency(myConcurrencyInt)`.
