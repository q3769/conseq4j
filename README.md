...
As long as the all incoming events carry some kind of correlatable information that can be used/converted as a seqence key, you can probably make use a conseq (and safeguard your beer). Some examples of the seqence key are order id, shipment id, ticket reservation id, session ids, etc.... Technically the sequence key can be any type of Object but good choices are identifiers that, after hashing, can group related events to the same hash code, and unrelated events into different hash codes, e.g. UUID, Long... The default hashing uses Guava; it should be good enough but otherwise, you can provide your own hasher in `ConcurrentSequentialExecutors.newBuilder().withBucketHasher(myHasher)` instead of `ConcurrentSequentialExecutors.newBuilder().withMaxConcurrency(myConcurrency)`.
