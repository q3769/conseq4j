...
- How spread-out the sequence keys' values are in the first place. (Obviously if all tasks carry the same sequence key, then only one executor will be running the tasks no matter how many executors are potentially available.)
