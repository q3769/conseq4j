...
submission threads, the tasks would be submitted in unpredictable order. The only sequence that matters to conseq4j is
the actual order in which the tasks are submitted to it. While multithreading, though, it may not be trivial for the
client to establish a definitive submission order since Java does not provide a strong guarantee on thread scheduling.
As a result of lacking definitive submission order in the first place, the sequencing capability of conseq4j will be
rendered moot.

Second, once a certain submission sequence is established by the API client, it is then conseq4j's concern and
